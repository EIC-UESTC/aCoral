...
#include "acoral.h"
#include "mem.h"
#define fs_malloc(size) buddy_malloc(size)
#define fs_free(ptr) buddy_free(ptr)
#define fs_printf acoral_print
...
#ifdef LFS_CRC
uint32_t lfs_crc(uint32_t crc, const void *buffer,
                 size_t size){return LFS_CRC(crc, buffer, size)}
#else
